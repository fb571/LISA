...
outdir = "LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16"
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/time_domain_signal.pdf")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/pure_frequency_domain_signal.pdf")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/frequency_domain_signal.pdf")
...
plt.tight_layout()
plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/snr_plot.pdf")
...
plt.grid(True, which='minor', linestyle=':', linewidth=0.5, alpha=0.4)
plt.tight_layout()
plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/snr_plot_data.pdf")
...
@jax.jit
def log_likelihood_fn(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, t, data_fft, dt,
                      window_jax, norm, noise2, freq_indices, n_vals):

    htd = - 2*R1 * jnp.cos(4*jnp.pi*f01*t + 2*phi01) - 2*R2 * jnp.cos(4*jnp.pi*f02*t + 2*phi02) - 2*R3 * jnp.cos(4*jnp.pi*f03*t + 2*phi03)
...
class FFTGWLikelihood(bilby.Likelihood):
    def __init__(self, t, data_fft, dt):
        super().__init__(parameters=dict(R1=None, f01=None, phi01=None, R2=None, f02=None, phi02=None, R3=None, f03=None, phi03=None))
        self.t = t
    ...
    def log_likelihood(self):
        ...
        return log_likelihood_jit(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03)

# ============================================================
# 6. Priors
# ============================================================

def convert_f01_f02_f03_to_df0s(parameters):
    """
...
priors = PriorDict(conversion_function=convert_f01_f02_f03_to_df0s)
...
param_order = ["R1", "f01", "phi01", "R2", "f02", "phi02", "R3", "f03", "phi03"]
...
if __name__ == "__main__":
    ...
    @jax.jit
    def gw_time_domain_jax_2(t, R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, n_vals):
        return - 2 * R1 * jnp.cos(4*jnp.pi*f01*t + 2*phi01) - 2 * R2 * jnp.cos(4*jnp.pi*f02*t + 2*phi02) - 2 * R3 * jnp.cos(4*jnp.pi*f03*t + 2*phi03)

    h_td_mle = gw_time_domain_jax_2(t, R1_mle, f01_mle, phi01_mle, R2_mle, f02_mle, phi02_mle, R3_mle, f03_mle, phi03_mle, n_vals)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/frequency_domain_signal_mle.pdf")
...
# ============================================================
    ...
    @jax.jit
    def compute_hfft_jax(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03):
        h_fft = dt * jnp.fft.rfft(gw_time_domain_jax_2(t, R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, n_vals) * window_jax) / norm
        return h_fft
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig("LISATripleCircFitEccentricModel,nlive=3000,SNRecc=16/posterior_draws_harmonics.pdf", dpi=150)