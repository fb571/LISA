...
outdir = "LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18"
...
true_Amp1 = (18/34.51281763790965) * prefactor1
true_peri1 = np.pi
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/time_domain_signal.pdf")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/pure_frequency_domain_signal.pdf")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/frequency_domain_signal.pdf")
...
plt.tight_layout()
plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/snr_plot.pdf")
...
plt.grid(True, which='minor', linestyle=':', linewidth=0.5, alpha=0.4)
plt.tight_layout()
plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/snr_plot_data.pdf")
...
@jax.jit
def log_likelihood_fn(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, R4, f04, phi04, R5, f05, phi05, t, data_fft, dt,
                      window_jax, norm, noise2, freq_indices, n_vals):

    htd = - 2*R1 * jnp.cos(4*jnp.pi*f01*t + 2*phi01) - 2*R2 * jnp.cos(4*jnp.pi*f02*t + 2*phi02) - 2*R3 * jnp.cos(4*jnp.pi*f03*t + 2*phi03) - 2*R4 * jnp.cos(4*jnp.pi*f04*t + 2*phi04) - 2*R5 * jnp.cos(4*jnp.pi*f05*t + 2*phi05)
...
class FFTGWLikelihood(bilby.Likelihood):
    def __init__(self, t, data_fft, dt):
        super().__init__(parameters=dict(R1=None, f01=None, phi01=None, R2=None, f02=None, phi02=None, R3=None, f03=None, phi03=None, R4=None, f04=None, phi04=None, R5=None, f05=None, phi05=None))
        self.t = t
    ...
    def log_likelihood(self):
        ...
        R3  = jnp.asarray(self.parameters["R3"])
        f03 = jnp.asarray(self.parameters["f03"])
        phi03 = jnp.asarray(self.parameters["phi03"])

        R4  = jnp.asarray(self.parameters["R4"])
        f04 = jnp.asarray(self.parameters["f04"])
        phi04 = jnp.asarray(self.parameters["phi04"])

        R5 = jnp.asarray(self.parameters["R5"])
        f05 = jnp.asarray(self.parameters["f05"])
        phi05 = jnp.asarray(self.parameters["phi05"])

        return log_likelihood_jit(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, R4, f04, phi04, R5, f05, phi05)

# ============================================================
# 6. Priors
# ============================================================

def convert_f01_f02_f03_f04_f05_to_df0s(parameters):
    """
    ...
    ----------
    parameters: dict
        Dictionary containing sampled parameter values, 'f01', 'f02', 'f03', 'f04'.

    Returns
    -------
    dict: Dictionary with constraint parameters 'df012', 'df023', and 'df034' added.
    """
    converted_parameters = parameters.copy()
    converted_parameters['df012'] = parameters['f02'] - parameters['f01']
    converted_parameters['df023'] = parameters['f03'] - parameters['f02']
    converted_parameters['df034'] = parameters['f04'] - parameters['f03']
    converted_parameters['df045'] = parameters['f05'] - parameters['f04']
    return converted_parameters

from bilby.core.prior import PriorDict, Uniform, Constraint
priors = PriorDict(conversion_function=convert_f01_f02_f03_f04_f05_to_df0s)
priors["R1"] = Uniform(0, 2e-21, name="R1", latex_label=r"$R_1$")
priors["f01"] = Uniform(0.0015, 0.01, name="f01", latex_label=r"$f_{0,1}\ \mathrm{[Hz]}$")
priors["phi01"] = Uniform(0, np.pi, name="phi01", latex_label=r"$\phi_{0,1}$")
priors["R2"] = Uniform(0, 2e-21, name="R2", latex_label=r"$R_2$")
priors["f02"] = Uniform(0.0015, 0.01, name="f02", latex_label=r"$f_{0,2}\ \mathrm{[Hz]}$")
priors["phi02"] = Uniform(0, np.pi, name="phi02", latex_label=r"$\phi_{0,2}$")
priors["R3"] = Uniform(0, 2e-21, name="R3", latex_label=r"$R_3$")
priors["f03"] = Uniform(0.0015, 0.01, name="f03", latex_label=r"$f_{0,3}\ \mathrm{[Hz]}$")
priors["phi03"] = Uniform(0, np.pi, name="phi03", latex_label=r"$\phi_{0,3}$")
priors["R4"] = Uniform(0, 2e-21, name="R4", latex_label=r"$R_4$")
priors["f04"] = Uniform(0.0015, 0.01, name="f04", latex_label=r"$f_{0,4}\ \mathrm{[Hz]}$")
priors["phi04"] = Uniform(0, np.pi, name="phi04", latex_label=r"$\phi_{0,4}$")
priors["R5"] = Uniform(0, 2e-21, name="R5", latex_label=r"$R_5$")
priors["f05"] = Uniform(0.0015, 0.01, name="f05", latex_label=r"$f_{0,5}\ \mathrm{[Hz]}$")
priors["phi05"] = Uniform(0, np.pi, name="phi05", latex_label=r"$\phi_{0,5}$")

priors["df012"] = Constraint(minimum=3/(N*dt), maximum=0.01, name="df012", latex_label=r"\Delta f_0\ \mathrm{[Hz]}")
priors["df023"] = Constraint(minimum=3/(N*dt), maximum=0.01, name="df023", latex_label=r"\Delta f_0\ \mathrm{[Hz]}")
priors["df034"] = Constraint(minimum=3/(N*dt), maximum=0.01, name="df034", latex_label=r"\Delta f_0\ \mathrm{[Hz]}")
priors["df045"] = Constraint(minimum=3/(N*dt), maximum=0.01, name="df045", latex_label=r"\Delta f_0\ \mathrm{[Hz]}")
...
param_order = ["R1", "f01", "phi01", "R2", "f02", "phi02", "R3", "f03", "phi03", "R4", "f04", "phi04", "R5", "f05", "phi05"]
...
if __name__ == "__main__":
    ...
    R3_mle = mle_parameters["R3"]
    f03_mle = mle_parameters["f03"]
    phi03_mle = mle_parameters["phi03"]

    R4_mle = mle_parameters["R4"]
    f04_mle = mle_parameters["f04"]
    phi04_mle = mle_parameters["phi04"]

    R5_mle = mle_parameters["R5"]
    f05_mle = mle_parameters["f05"]
    phi05_mle = mle_parameters["phi05"]

    @jax.jit
    def gw_time_domain_jax_2(t, R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, R4, f04, phi04, R5, f05, phi05, n_vals):
        return - 2 * R1 * jnp.cos(4*jnp.pi*f01*t + 2*phi01) - 2 * R2 * jnp.cos(4*jnp.pi*f02*t + 2*phi02) - 2 * R3 * jnp.cos(4*jnp.pi*f03*t + 2*phi03) - 2 * R4 * jnp.cos(4*jnp.pi*f04*t + 2*phi04) - 2 * R5 * jnp.cos(4*jnp.pi*f05*t + 2*phi05)

    h_td_mle = gw_time_domain_jax_2(t, R1_mle, f01_mle, phi01_mle, R2_mle, f02_mle, phi02_mle, R3_mle, f03_mle, phi03_mle, R4_mle, f04_mle, phi04_mle, R5_mle, f05_mle, phi05_mle, n_vals)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/frequency_domain_signal_mle.pdf")
...
# ============================================================
    ...
    @jax.jit
    def compute_hfft_jax(R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, R4, f04, phi04, R5, f05, phi05):
        h_fft = dt * jnp.fft.rfft(gw_time_domain_jax_2(t, R1, f01, phi01, R2, f02, phi02, R3, f03, phi03, R4, f04, phi04, R5, f05, phi05,n_vals) * window_jax) / norm
        return h_fft
    ...
    for i, row in posterior_samples.iterrows():
        h_fft_i = compute_hfft_jax(
            ...
            jnp.float64(row["R3"]),
            jnp.float64(row["f03"]),
            jnp.float64(row["phi03"]),
            jnp.float64(row["R4"]),
            jnp.float64(row["f04"]),
            jnp.float64(row["phi04"]),
            jnp.float64(row["R5"]),
            jnp.float64(row["f05"]),
            jnp.float64(row["phi05"]),
        )
        ...
        idx_circ_3 = np.argmin(np.abs(freqs - 2*f03_i))
        circ_amp_3 = amps[idx_circ_3]

        idx_circ_4 = np.argmin(np.abs(freqs - 2*f04_i))
        circ_amp_4 = amps[idx_circ_4]

        idx_circ_5 = np.argmin(np.abs(freqs - 2*f05_i))
        circ_amp_5 = amps[idx_circ_5]

        ax.scatter([2*f01_i], [circ_amp_1], s=8, alpha=0.3, color='tomato')
        ax.scatter([2*f02_i], [circ_amp_2], s=8, alpha=0.3, color='tomato')
        ax.scatter([2*f03_i], [circ_amp_3], s=8, alpha=0.3, color='tomato')
        ax.scatter([2*f04_i], [circ_amp_4], s=8, alpha=0.3, color='tomato')
        ax.scatter([2*f05_i], [circ_amp_5], s=8, alpha=0.3, color='tomato')
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig("LISAQuintCircFitEccentricWithCircularModel,nlive=5000,0.9SNRcirc=SNRecc=18/posterior_draws_harmonics.pdf", dpi=150)